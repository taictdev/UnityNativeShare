...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureGUI : MonoBehaviour
	{
		...
		private int selectedTransparency = 0;
		...
		private void MyWindow(int id)
		{
			...
			if (_movieCapture != null)
			{
				...
				else if (_movieCapture.OutputTarget == OutputTarget.ImageSequence)
				{
					...
					// edit the quality and the transparency from the ingame GUI editor
#if UNITY_EDITOR_WIN
					if (_movieCapture.NativeImageSequenceFormat == ImageSequenceFormat.PNG) // PNG 
					{
						// quality not an option when using FPNG
						//GUILayout.Label("Quality");
						//GUILayout.BeginHorizontal();
						//_movieCapture.GetEncoderHints().imageHints.quality = GUILayout.HorizontalSlider(
						//	(int)_movieCapture.GetEncoderHints().imageHints.quality, 0, 100);
						//GUILayout.Label(_movieCapture.GetEncoderHints().imageHints.quality.ToString());
						//GUILayout.EndHorizontal();
						bool val = _movieCapture.GetEncoderHints().imageHints.quality >= 1 ? true : false;
						...
						selectedTransparency = GUILayout.SelectionGrid(selectedTransparency, new string[] { "None", "Codec", "Top Bottom", "Left Right" }, 4);
						_movieCapture.GetEncoderHints().imageHints.transparency = (Transparency)selectedTransparency;
						GUILayout.EndHorizontal();
					}
					else // JPEG
					{
						if (_movieCapture.GetEncoderHints().imageHints.quality > 1)
							_movieCapture.GetEncoderHints().imageHints.quality = 1;
						GUILayout.Label("Quality");
						GUILayout.BeginHorizontal();
						_movieCapture.GetEncoderHints().imageHints.quality = GUILayout.HorizontalSlider(
							_movieCapture.GetEncoderHints().imageHints.quality, 0.1f, 1f);
						GUILayout.Label(_movieCapture.GetEncoderHints().imageHints.quality.ToString());
						GUILayout.EndHorizontal();
					}
#endif
